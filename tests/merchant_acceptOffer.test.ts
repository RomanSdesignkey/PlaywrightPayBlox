...
test('Accepting and signing offer', async ({ page }) => {
  ...
  await allure.step('Verify Additional Information', async () => {
    await page.waitForSelector(
      'h2:has-text("Processor requires additional information")',
    );
    await expect(
      page.locator('h2:has-text("Processor requires additional information")').first(),
    ).toBeVisible();
  });
  ...
  await allure.step('Click on Today', async () => {
    const acceptButton = page.locator(
      '.date-container-days[data-action="selectDay"] .day.today',
    );
    await acceptButton.first().click();
  });
  ...
  await allure.step('Fill Monthly Visa/MC/Discovery Volume', async () => {
    const acceptButton = page.locator(
      'input[field_name="Monthly Visa/MC/Discovery Volume"]',
    );
    await acceptButton.first().fill('1000');
  });
